...
export class UserProfileTimelineComponent implements OnInit {
  ...
  StartOfPage() {
    ...
    // }
    //profile pic Cropper settings 2
    ...
    // change method of cover pic box
    // $("#update-cover-pic-modal input[type=file]").change(function(e){ 
    //   $("#update-cover-pic-modal canvas").show();      
    // });
  }
  ...
  getMyPost() {
    ...
    if (getUserId != null) {
      let dataForForm = {
        ...
        showPostToUser: ['Public', 'Cohorts', 'Private']
      }
  ...
  coverPhotosDropped($event) {
    alert("Dd")
    $('.profile-pic-cropper-box').show();
    // console.log("event : ", $event);
    this.coverPhoto = $event.target.files;
  ...
  submitProfilePic() {
    if (this.profilePhoto == undefined) {
      ...
      this.loading = true;
      let cropped_pic;
      console.log("this dat :", this.profile_pic_cropper_data);
      fetch(this.profile_pic_cropper_data.image).then(res => res.blob()).then(blob => {
        cropped_pic = new File([blob], this.profilePhotoName, { type: "image/png" })
        console.log("file : ", cropped_pic);
  ...
  submitCoverPic() {
    ...
    let coverImageName = 'cover_image' + out;
  ...
  hideImageCropper() {
    $('.profile-pic-cropper-box').hide();
    $('#update-cover-pic-modal input[type=file]').css('margin-bottom','10px');
    // $("#update-cover-pic-modal canvas").hide();
  }
}
